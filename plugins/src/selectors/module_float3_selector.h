...
//---DATA-STORAGE---------------------------------------------------------------
  ...
  std::vector<vsx_module_param_float3*> float_x;
...
public:
  ...
  void redeclare_in_params(vsx_module_param_list& in_parameters)
  {
    ...
    for(int i = 0; i <= i_prev_inputs; ++i)
    {
        ...
        float_x.push_back((vsx_module_param_float3*)in_parameters.create(VSX_MODULE_PARAM_ID_FLOAT3, i_paramName.str().c_str()));
        float_x[i]->set(0.0, 0);
        float_x[i]->set(0.0, 1);
        float_x[i]->set(0.0, 2);
    }
