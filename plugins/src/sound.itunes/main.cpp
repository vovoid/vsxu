...
void normalize_fft(float* fft, vsx_float_array& spectrum) {
  float B1 = (float)pow(8.0,1.0/512.0); //1.004
  float dd = (float)(1.0*(512.0/8.0));
  float a = 0;
  float b;
  float diff = 0;
  int aa;
  for (int i = 1; i < 512; ++i) {
    ...
    {
      ++aa;
      (*(spectrum.data))[i] += fft[aa]*3 * ((float)ceil(a) - a);
      while (aa != (int)b) {
        (*(spectrum.data))[i] += fft[aa]*3;
        ++aa;
      }
      (*(spectrum.data))[i] += fft[aa+1]*3 * (b - (float)floor(b));
    }
    a = b;
  }
}
...
void declare_params(vsx_module_param_list& in_parameters, vsx_module_param_list& out_parameters)
{
  ...
  for (int ii = 0; ii < 512; ++ii) wave.data->push_back(0);
  wave_p->set_p(wave);
  ...
  for (int ii = 0; ii < 512; ++ii) spectrum.data->push_back(0);
  for (int ii = 0; ii < 512; ++ii) spectrum_hq.data->push_back(0);
...
int echo_log(const char* messagel, int a) {
...
void run() {
  ...
  if (0 == engine->param_float_arrays.size())
  {
    int ii;
    for (ii = 0; ii < 512; ++ii) {
        (*(wave.data))[ii] = (float)(rand()%1000) * 0.0005f * l_mul;
    }
    wave_p->set_p(wave);

    for (ii = 0; ii < 512; ++ii) {
        (*(spectrum.data))[ii] = (float)(rand()%1000) * 0.0005f * l_mul;
    }
  } else
  {
    ...
    for (ii = 0; ii < 512; ++ii) {
        (*(wave.data))[ii] = (*lv_wave_data).array[ii] * l_mul;
    }
    wave_p->set_p(wave);

    for (ii = 0; ii < 512; ++ii) {
        (*(spectrum.data))[ii] = (*lv_freq_data).array[ii] * l_mul;
    }
  }
  spectrum_p->set_p(spectrum);
...
#define spec_calc(obj, start) \
  for (i = start * 64; i < (start+1)*64; i++) {\
    cur_val += (*(spectrum.data))[ (int)round((float)i * 0.5f) ];\
  }\
