...
unsigned long vsx_module_plugin::getColor(float u, float v) {
...
void vsx_module_plugin::skyBoxFromMap(int plane) {
	int texSize = 512;
...
"bitmap:bitmap";
  ...
  info->out_param_spec = "render_out:render,bitmaps:complex{positive_x:bitmap,negative_x:bitmap,positive_y:bitmap,negative_y:bitmap,positive_z:bitmap,negative_z:bitmap}";
  info->component_class = "render";
}


void vsx_module_plugin::declare_params(vsx_module_param_list& in_parameters, vsx_module_param_list& out_parameters)
{
  mapMode = MAP_SPHERE;
...
typedef struct {
  vsx_module_plugin* target;
  int p_plane;
} SB_THREAD_DATA;

void* worker(void *data) {
  //printf("skybox thread starting, plane is: %d\n",((SB_THREAD_DATA*)data)->p_plane);
  ((SB_THREAD_DATA*)data)->target->skyBoxFromMap(((SB_THREAD_DATA*)data)->p_plane);
//  ((SB_THREAD_DATA*)data)->target->
  //printf("skybox thread done, plane is: %d\n",((SB_THREAD_DATA*)data)->p_plane);
  // the thread will die here.
  delete (SB_THREAD_DATA*)data;
  return 0;
}


void vsx_module_plugin::run() {
  //printf("skybox run\n");
...
void vsx_module_plugin::output(vsx_module_param_abs* param) {
  if (loading_done)
  {
...
void vsx_module_plugin::on_delete() {
