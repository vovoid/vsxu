...
FMOD_RESULT F_CALLBACK pcm_read( FMOD_SOUND *  sound,  void *  data,  unsigned int  datalen)
{
	printf("called back\n");
}
...
//    driver = 25;
    ...
    */
    ...
    exinfo.pcmreadcallback = &pcm_read;
    exinfo.cbsize           = sizeof(FMOD_CREATESOUNDEXINFO);
    exinfo.numchannels      = 2;
    exinfo.format           = FMOD_SOUND_FORMAT_PCM16;
    exinfo.defaultfrequency = 48000;
//    exinfo.length           = exinfo.defaultfrequency * sizeof(short) * exinfo.numchannels * 5;
    exinfo.length = sizeof(short) * 2048;
...
//                    for (int i = 0; i < 10000; ++i)
                    //while (1)
                    {
                    	...
                    	void* pointer1;
                    	void* pointer2;
                    	unsigned int read_bytes2 = 0;
                    	//sleep(1);
                    	//if (FMOD_Sound_ReadData(sound,(void*)&buf, 64*sizeof(short), &read_bytes) != FMOD_OK)
                    	if (FMOD_Sound_Lock(sound, 0, 1024 * sizeof(short), &pointer1,&pointer2,&read_bytes, &read_bytes2) != FMOD_OK)
                    		printf("error agh\n");
                    	printf("read %d bytes  - ",read_bytes);
                    	printf("read %d bytes  - ",read_bytes2);
                    	int i;
                    	for (i = 0; i < 8; ++i)
                    	printf("%d,", ((short*)pointer1)[i] );
                    	printf("\n");
                    	FMOD_Sound_Unlock(sound, pointer1, pointer2, read_bytes, read_bytes2);
                    	//usleep(100);
                    }
                    break;
                }
