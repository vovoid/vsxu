...
class input_audio_raw : public vsx_module
{
                     ...
                     double streamTime, RtAudioStreamStatus status, void *userData )
  {
    ...
    if ( status )
    {
      printf("Stream overflow/underflow detected!\n");
    }
    else
    {
      ((input_audio_raw*)userData)->read_data((sample*)inputBuffer, nBufferFrames);
    }
...
RtAudio* input_audio_raw::m_adc = 0;
