...
vsx_font_info* vsx_font::load(vsx_string<>font, vsxf* filesystem)
{
  ...
  if (font[0] != '-')
  {
    ...
    my_font_info->texture = vsx_texture_loader::load( base_path+font, filesystem, true, vsx_texture_gl_hint(true, false, true, true) );
    ch = 16.0f/255.0f;
    cw = 10.0f/255.0f;
  }
  else
...
void vsx_font::reinit(vsx_font_info* f_info, vsx_string<>font, vsxf* filesystem)
{
  if (f_info->type == 0)
  {
    f_info->texture = vsx_texture_loader::load(base_path+font, filesystem, true, vsx_texture_gl_hint(true, false, true, true));
    return;
  }
