...
//        printf("token %d: %s\n",i,token.c_str());
		...
		#define fprintf(a,b,c)
            ...
            fprintf(stderr, 
                    "RenderTexture Warning: mistake in components definition (rgb + %d).\n", 
                    _iNumComponents);
        ...
        if (kv.first == "rgba" && (_iNumComponents == 0))
        {
            ...
            fprintf(stderr, 
                    "RenderTexture Warning: mistake in components definition (rgba + %d).\n", 
                    _iNumComponents);
        ...
        if (kv.first == "r" && (_iNumComponents <= 1))
        {
            ...
            fprintf(stderr, 
                    "RenderTexture Warning: mistake in components definition (r + %d).\n", 
                    _iNumComponents);
...
*/ 
RenderTexture::KeyVal RenderTexture::_GetKeyValuePair(vsx_string token)
{
    int pos = 0;
    if ((pos = token.find("=")) != -1)
    {
        vsx_string key = token.substr(0, pos);
        vsx_string value = token.substr(pos+1, token.size()-pos+1);
...
*/ 
vector<int> RenderTexture::_ParseBitVector(vsx_string bitVector)
{
    vector<vsx_string> pieces;
    vector<int> bits;
    ...
    do
    { 
        nextpos = bitVector.find(", \n", pos);
        vsx_string foo = bitVector.substr(pos,nextpos-pos);
        pieces.push_back(foo); 
        pos = nextpos+1; 
    } while (nextpos != -1 );

    for ( vector<vsx_string>::iterator it = pieces.begin(); it != pieces.end(); it++)
    {
...
bool RenderTexture::_VerifyExtensions()
{
    ...
    {
      char msg[] = "GLX_SGIX_pbuffer";
      PrintExtensionError(msg);
      return false;
    }
    if (!GLXEW_SGIX_fbconfig)
    {
      char msg[] = "GLX_SGIX_fbconfig";
      PrintExtensionError(msg);
      return false;
    }
    if (_bIsDepthTexture && !GLEW_ARB_depth_texture)
    {
      char msg[] = "GL_ARB_depth_texture";
      PrintExtensionError(msg);
      return false;
    }
    if (_bFloat && _bIsTexture && !GLXEW_NV_float_buffer)
    {
      char msg[] = "GLX_NV_float_buffer";
      PrintExtensionError(msg);
      return false;
    }
    if (_eUpdateMode == RT_RENDER_TO_TEXTURE)
    {
      char msg[] = "Some GLX render texture extension: FIXME!";
      PrintExtensionError(msg);
      return false;
    }
#endif
