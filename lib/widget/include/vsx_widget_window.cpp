...
void vsx_widget_window::init() {
  ...
  button_close = dynamic_cast<vsx_widget_button*>( add(new vsx_widget_button,"bc1") );
  init_children();
  init_run = true;
  ...
  button_close->on_click = [this](){ on_close_click(); };
}
...
void vsx_widget_window::i_draw() {
  ...
  font.print(vsx_vector3<>((pos.x+font_size*0.1f)*screen_aspect,pos.y+size.y-font_size*0.85f)," " + title,font_size*0.4f);
}
