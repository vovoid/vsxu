		...
		vsx_widget_coords coords)
{
	...
	if (extra_hit != -1) // extra hit = bezier spline handles
	{
      ...
      /*items[mouse_clicked_id].handle1.x = f / items[mouse_clicked_id].total_length;
      ...
      items[mouse_clicked_id].handle1.y = y	- s2f(items[mouse_clicked_id].value);*/
		}
		else if (extra_hit == 2)
		{
      ...
      server_message("pseq_r update",
        base64_encode( items[mouse_clicked_id].get_value() ) + " " +
    ...
    // delta time calculations for updating the engine
        ...
        if (!is_controller)
          ...
          server_message("pseq_r update",
            base64_encode( items[mouse_clicked_id - 1].get_value() ) + " " +
  ...
  if (t->cmd == "menu_interp_keyboard_value")
	{
      ...
      items[mouse_clicked_id].set_value( t->cmd_data );
        ...
        server_message("pseq_r update",
          base64_encode(
            items[mouse_clicked_id].get_value()) + " " +
                       f2s( items[mouse_clicked_id].get_total_length()) + " " +
                       i2s( items[mouse_clicked_id].get_interpolation()) + " " +
            i2s( mouse_clicked_id));
			else
	...
	if (t->cmd == "menu_interp")
	{
		if (mouse_clicked_id != -1)
		{
      items[mouse_clicked_id].set_interpolation( vsx_string_aux::s2i(t->cmd_data) );
