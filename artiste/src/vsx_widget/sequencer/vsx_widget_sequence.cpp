...
void vsx_widget_sequence_editor::init() {
  ...
  sequence_tree->init();
	...
	sequence_tree->sequence_editor = this;
...
void vsx_widget_sequence_editor::toggle_channel_visible(vsx_string name) {
	if (channels_map.find(name) != channels_map.end())
	{
		...
		//printf("status: %d\n", ((vsx_widget_seq_channel*)channels_map[name])->hidden_by_sequencer);
	}
	else {
...
void vsx_widget_sequence_editor::check_timeline() {
  //if (engine_status == VSX_ENGINE_PLAYING) {
    if (curtime < tstart) {
...
void vsx_widget_sequence_editor::vsx_command_process_b(vsx_command_s *t) {
  ...
  // sequence list operations
 	if (t->cmd == "pseq_l_dump_ok") {
 		if (t->parts.size() > 1)
 		{
 			printf("pseq_l_dump_ok run in gui\n");
			vsx_string deli = "&";
			vsx_avector<vsx_string> parts;
			explode(t->parts[1], deli, parts);
  ...
  if (t->cmd == "pseq_p_ok") {
    ...
    if (t->parts[1] == "inject_get") {
      if (channels_map.find(t->parts[2]+":"+t->parts[3]) != channels_map.end()) {
        //printf("injecting gui string\n");
        command_q_b.add(t);
        channels_map[(t->parts[2]+":"+t->parts[3])]->vsx_command_queue_b((vsx_widget*)this);
      } else {
      	printf("trying to init the sequence instead\n");
        t->parts[1] = "init";
        show_after_init = true;
      }
    }
    if (t->parts[1] == "list" || t->parts[1] == "init") {
    	printf("sequence widget got: %s\n", t->parts[1].c_str());
      //printf("list of channels: %s\n", t->parts[2].c_str());
      if (channels_map.find(t->parts[2]+":"+t->parts[3]) == channels_map.end()) {
